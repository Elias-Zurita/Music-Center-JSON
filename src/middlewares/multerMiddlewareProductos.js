...
const storage = multer.diskStorage({   // Alojamiento de archivos
    ...
    filename: (req, file, cb) => { // Propiedad que guarda el nombre de la imagen // 
        const newFilename = "imagen" + Date.now() + path.extname (file.originalname); // El nombre del archivo va a ser "imagen + el horario en mili segundos (Datenow) + extension (extname)" 
        cb (null, newFilename); 
    } 
}); 
